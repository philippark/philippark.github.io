---
layout: page
permalink: /about
---

Hi, I'm Philip Park. 

Currently, I'm a Software Engineer @ Google, working in Core AI Foundations based in the NYC office. I work as a backend engineer in a voice and messaging infrastructure team.

I got my bachelors in computer science from Rensselaer Polytechnic Institute (RPI) with honors (Cum Laude).

Professionally:
I'm interested in backend engineering, computer systems, C++, distributed systems, and AI engineering. 

Personally:
I'd describe myself as an autodidact, a forever learner and student, an avid reader, and ambitious. I absolutely love a good struggle of solving a problem, whether it's a coding problem, a chess problem, or another mental sport. 

In the past I was a 
- Swe intern @ Google. same team as my full-time. 
- Swe intern @ a NYC based SAAS startup
- RPI CS tutor - held open office hours for 2 freshmen courses: Intro to CS, and Data Structures. 
- President of RPI's Chess Club - helped found this club from the ground up, discord mod for a 400+ member server, otb tournament manager.
- Chess.com College Ambassador - basically a glorified free Chess.com merch and membership distributor to my fellow students. 
- Member of UPE (Cs Honor's Society) - helped review resumes during career fairs, wrote some problems for our university competitive programming contest, etc.
- Open source developer for RCOS (Rensselaer center for open source) - a pretty well known aspect of my university is our open source community. 

In my free time, I write, jam with my guitar, and read. 

Online Profiles:

- [Leetcode](https://leetcode.com/u/philippark271/)
- [Chess.com](https://www.chess.com/member/monkeydpark)
- [Codeforces](https://codeforces.com/profile/philipdpark)